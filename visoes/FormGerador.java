...
import visoes.componentes.ItemTreePanel;
...
public class FormGerador extends JPanel {
  ...
  private void construirFormulario() {
    ...
    for (Field field: clazz.getDeclaredFields()) {
      ...
      if (!meta.dependeDe().isEmpty()) {
        dependencias.computeIfAbsent(meta.dependeDe(), k -> new ArrayList<>()).add(field.getName());
      }
  ...
  private void preencherValorInicial(Field field, Component campo) {
    ...
    if (instanciaInicial == null) return;
    ...
    try {
      ...
      } else if (campo instanceof JPanel panel) {
  ...
  private JComponent criarCampo(Field field, CampoFormulario meta) {
    ...
    if (meta.tipo() == TipoCampo.COMBO) {
      ...
      try {
        ...
        if (meta.provider() != ComboDadosProvedor.class) {
          ...
          ComboDadosProvedor provider = meta.provider().getDeclaredConstructor().newInstance();
          ...
          for (Enum<?> e: enumClass.getEnumConstants()) {
            ComboItem item = new ComboItem(e.name(), formatEnum(e.name()), null);
            itens.add(item);
            combo.addItem(item);
          }
          ...
          for (String opcao: meta.opcoes()) {
            ComboItem item = new ComboItem(null, opcao, null);
            itens.add(item);
            combo.addItem(item);
          }
        }
    ...
    if (meta.tipo() == TipoCampo.DATA)
    {
      JTextFieldData data = new JTextFieldData("Data");

      JPanel panel = new JPanel(new BorderLayout(5, 0));
      panel.add(data.getDTestField(), BorderLayout.CENTER);
      panel.add(data.getDButton(), BorderLayout.EAST);

      panel.setPreferredSize(new Dimension(meta.largura(), meta.altura()));

      panel.setMaximumSize( new Dimension(Integer.MAX_VALUE, meta.altura()));

      return panel;
    }
    if (meta.tipo() == TipoCampo.TREE_CHECK) {
      try {

        ComboDadosProvedor provider = meta.provider().getDeclaredConstructor().newInstance();

        List <ComboItem> itens = provider.carregar();

        ItemTreePanel painel = new ItemTreePanel(itens);

        painel.setPreferredSize(new Dimension(meta.largura(), meta.altura()));
  ...
  private Object obterValor(JComponent component) {
    ...
    if (component instanceof ItemTreePanel tree) {
        return tree.getIdsSelecionados();
    }
    return null;
  }
  ...
  public boolean validarFormulario() {
    ...
    for (var campo: campos.entrySet()) {
      ...
      if (meta.obrigatorio()) {
        ...
        if (vazio) {
          erros.append("- ").append(meta.descricao()).append("\n");
        }
      }
    }
    ...
    if (!erros.isEmpty()) {
      ...
      this, "Campos obrigatórios:\n\n" + erros, "Validação", JOptionPane.WARNING_MESSAGE);
  ...
  private void executarAcao(String nomeAcao) {
    ...
    if (acao == null) {
      ...
      JOptionPane.showMessageDialog(
      this, "Nenhuma ação definida para: " + nomeAcao);
    ...
    try {
      ...
      Map < String,
      Object > dados = getDadosFormulario();
      ...
      for (var entrada: dados.entrySet()) {
        ...
        String setter = "set" + Character.toUpperCase(campo.charAt(0)) + campo.substring(1);
      ...
      JOptionPane.showMessageDialog(
      this, "Erro ao executar ação: " + e.getMessage());
    }
  }
    ...
    editor.getDocument().addDocumentListener(new DocumentListener() {
      ...
      private void pesquisar() {
        ...
        SwingUtilities.invokeLater(() ->{
          ...
          List<ComboItem> itens = dadosCombos.getOrDefault(
          nomeCampo, Collections.emptyList());

          combo.removeAllItems();

          for (ComboItem item: itens) {

            if (texto.isEmpty() || item.getDescricao().toLowerCase().contains(texto)) {
