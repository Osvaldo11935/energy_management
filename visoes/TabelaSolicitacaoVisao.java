...
public class TabelaSolicitacaoVisao extends JFrame {
  ...
  private void carregarTabela() {
    ...
    solicitacoes = usuarioLogado.ehTecnico() ? arquivo.buscarPorTecnicoResponsavelId(usuarioLogado.getId()) : arquivo.buscarPorUsuarioId(usuarioLogado.getId());
    ...
    colunas, dados, criarAcoes(), "Novo", this::abrirNovoSolicitacao);
  ...
  private List<TabelaGerador.AcaoTabela> criarAcoes() {
    ...
    acoes.add(new TabelaGerador.AcaoTabela() {@Override
      public void executar(int linha, Object[] dadosLinha) {
        ...
        FormWindow tela = new FormWindow("Atualizar Solicitacao", 456, 396, new FormSolicitacaoVisao(Solicitacao));
    ...
    acoes.add(new TabelaGerador.AcaoTabela() {@Override
      public void executar(int linha, Object[] dadosLinha) {
        ...
        TabelaSolicitacaoVisao.this, "Remover " + dadosLinha[1] + "?", "Confirmação", JOptionPane.YES_NO_OPTION);
        ...
        if (confirm == JOptionPane.YES_OPTION) {
          ...
          JOptionPane.showMessageDialog(TabelaSolicitacaoVisao.this, "Removido!");
        }
      }
    ...
    acoes.add(new TabelaGerador.AcaoTabela() {@Override
      public void executar(int linha, Object[] dadosLinha) {
        ...
        SolicitacaoModelo Solicitacao = buscarSolicitacao(dadosLinha);
        FormWindow tela = new FormWindow("Interação na Solicitacao", 600, 400, new SolicitacaoDetalheVisao(Solicitacao));
  ...
  Object[] dadosLinha) {
    ...
    int id = getId(dadosLinha);
    return solicitacoes.stream().filter(p -> p.getId() == id).findFirst().orElse(null);
  }
  ...
  private void abrirNovoSolicitacao() {
    ...
    FormWindow tela = new FormWindow("Nova Solicitacao", 456, 396, new FormSolicitacaoVisao());
  ...
  public static void main(String[] args) {
    SwingUtilities.invokeLater(() -> new TabelaSolicitacaoVisao().setVisible(true));
  }
}