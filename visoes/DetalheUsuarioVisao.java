...
public class DetalheUsuarioVisao extends JFrame {
	...
	public DetalheUsuarioVisao(Supplier < String > idUsuarioProvider) {
	...
	private void abrirFormulario() {
		...
		if(tabs.getSelectedIndex() == 0) {
			tela = new FormWindow("Dados Pessoais", 456, 510, new FormPessoaVisao(
				() -> String.valueOf(usuarioId)));
		} else {
			tela = new FormContratoVisao(usuarioId);
		}
	...
	private void montarAbas() {
		...
		DetalhePessoaVisao dados = new DetalhePessoaVisao();
		...
		if(usuario.ehCliente()) {
			...
			if(clientes != null && !clientes.isEmpty()) {
				DetalheContratoVisao contrato = new DetalheContratoVisao();
