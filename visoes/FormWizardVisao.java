...
public class FormWizardVisao extends JFrame {
    ...
    public FormWizardVisao() {
        ...
        painelCards.add(new FormUsuarioVisao(id -> {
            this.idUsuario = id;
    ...
    private void configurarProximosPassos() {
        ...
        if (usuario.ehCliente()) {
            ...
            painelCards.add(new FormClienteVisao(() -> idUsuario, clienteId ->
            {
                painelCards.add(new FormContadorVisao(() -> clienteId),"PASSO_3");
                painelCards.add(new FormPessoaVisao(() -> idUsuario),"PASSO_4");
                totalPassos = 4;
            }),"PASSO_2");


        } else {
            painelCards.add(new FormPessoaVisao(() -> idUsuario),"PASSO_2");
            totalPassos = 2;
        }
    }

    private void avancar() {
        if (passoAtual == 0 && (idUsuario == null || idUsuario.isBlank())) {
            JOptionPane.showMessageDialog(this,"Primeiro deve concluir o cadastro do usuário.");
            return;
        }
    ...
    private void finalizar() {
        JOptionPane.showMessageDialog(this, "Cadastro concluído! ID do usuário: " + idUsuario);
        dispose();
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(() ->new FormWizardVisao().setVisible(true));
    }
}