...
public class DetalheContrato {
    ...
    private BotaoSecao[] botoesContrato(ContadorModelo contador, ClienteModelo cliente) {
        ...
        {
            if(cliente.getTipoContrato().toLowerCase().equals(TipoContrato.POS_PAGO.toString())){
                botoes.add(new BotaoSecao("Faturas", e -> {new TabelaFatura(cliente.getId()).setVisible(true);}));
            }
            else{
                botoes.add(new BotaoSecao("Saldo", e -> {new TabelaFatura(cliente.getId()).setVisible(true);}));
            }
        }
        else
        {
                ...
                botoes.add(
                    new BotaoSecao("Novo Contador", e -> {
                        abrirTela(
                            ...
                            456,
                            300,
                            new FormContador(() -> String.valueOf(cliente.getId()))
                        );
                    })
                );
            }
